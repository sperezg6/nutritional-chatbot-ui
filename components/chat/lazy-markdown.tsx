...
// Note: Uses dark text on light background
const markdownComponents = {
  h1: ({ node, ...props }: any) => (
    <h1 className="text-xl font-semibold text-gray-900 mt-6 mb-4 pb-3 border-b border-gray-200" {...props} />
  ),
  h2: ({ node, ...props }: any) => (
    <h2 className="text-lg font-semibold text-gray-900 mt-6 mb-3 pb-2 border-b border-gray-200" {...props} />
  ),
  h3: ({ node, ...props }: any) => (
    <h3 className="text-base font-semibold text-gray-900 mt-5 mb-2" {...props} />
  ),
  ul: ({ node, ...props }: any) => (
  ...
  li: ({ node, children, ...props }: any) => (
    <li className="pl-1 text-gray-700" {...props}>{children}</li>
  ),
  a: ({ node, href, children, ...props }: any) => (
    <a
      ...
      className="text-[#4DBDC9] hover:text-[#3A9BA6] underline font-medium"
      {...props}
    >
      {children}
    </a>
  ...
  p: ({ node, children, ...props }: any) => (
    <p className="text-gray-700 my-2 leading-relaxed" {...props}>{children}</p>
  ),
  strong: ({ node, children, ...props }: any) => {
    const text = String(children)
    if (text.endsWith(':')) {
      return (
        <strong className="block mt-4 mb-2 text-gray-900 font-semibold" {...props}>
          {children}
        </strong>
      )
    }
